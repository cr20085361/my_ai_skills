...
The project is optimized for safe coding-agent use: deployment is preview-first, generated and temporary artifacts are ignored, and tests only collect the repository's own test suite.
...
```powershell
.\deploy.ps1              # dry-run
.\deploy.ps1 -Apply       # real global deployment
```

```bash
./deploy.sh               # dry-run
./deploy.sh --apply       # real global deployment
```
